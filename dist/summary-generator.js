...
exports.generateRichSummary = generateRichSummary;
function generateRichSummary(ctx) {
    return {
...
function buildExecutiveSummary(ctx) {
    ...
    lines.push('');
    lines.push(`This PR touches **${ctx.files_changed} file(s)** with **+${ctx.lines_added}/-${ctx.lines_removed}** lines across ${ctx.change_summary.affected_areas.length > 0 ? ctx.change_summary.affected_areas.map((a) => `\`${a}\``).join(', ') : 'the codebase'}.`);
...
function buildChangeBreakdown(ctx) {
...
function buildRiskAssessment(summary) {
    const riskEmoji = { low: '🟢', medium: '🟡', high: '🔴' }[summary.risk_level];
    const lines = [`### Risk Assessment: ${riskEmoji} ${capitalize(summary.risk_level)}`];
    lines.push(`**Complexity score:** ${summary.complexity_score}/10`);
    lines.push('');
    const flags = [];
    if (summary.has_migrations)
        flags.push('⚠️ Contains database migrations — review carefully');
    if (!summary.has_tests)
        flags.push('⚠️ No test files detected — consider adding coverage');
    ...
    if (summary.has_config)
        flags.push('ℹ️ Configuration files modified — check environment compatibility');
    if (flags.length > 0) {
        lines.push('**Flags:**');
...
function buildReviewChecklist(ctx) {
    const checklist = [
...
function buildAIReviewerPrompt(ctx) {
